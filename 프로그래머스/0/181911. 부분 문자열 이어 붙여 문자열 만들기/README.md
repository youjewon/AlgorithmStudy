...
>
#p.s
str.substr으로 접근하는 문제로 n번째 자리에서 m개의 갯수를 가져오기 때문에 parts의 값을 그대로 s e 로 가져오면 배열을 넘어서 가져오는 경우가 발생한다. 
뭔하는 위치에서 원하는 만큼 가져오기 위해서는 s - e + 1개를 사용하여 원하는 위치에 원하는 갯수를 가져올 수 있다.

ex) [ |3|"ahocorasick"|[7, 7]|"s"| 같은경우 필요한 문자가 s 한문자 만 필요한데 substr(7,7)로 가져오게 되면 sick를 가져오게 된다. 이를 해결하기 위한 s - e + 1 개로 가져와야 s 한글자 가져옴옴
